...
- Comptime branch budget: the config parse, regex compilation, trie build and dispatch assignment share the per-compilation comptime branch quota (default 100000, `-Dconfig_branch_quota=<n>`). `Config.comptimeValidate` measures the config's compile cost and fails with a clear error before the quota is exhausted (safety factor 1.5); calibrate against the raw quota error when the units change (§9 of `docs/plan.md`).
