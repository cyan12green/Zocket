...
- `http/` currently: `parser.zig` (incremental HTTP/1.x request parser: request line, headers, Content-Length body, keep-alive logic, 400/431/413/501 outcomes), `response.zig` (status + headers + body builder, Content-Length always set, `writeToBuffer` for the send path, single-pass serialisation with `formatUInt`, `writevParts`/`writevHeadParts` emitting the whole response as zero-copy writev slices — no head concatenation), `header_dfa.zig` (comptime-built DFA classifying header names to an exact `HeaderTag` — one table lookup per byte, terminal state is the tag; the parser stores tags in slots and `Request.header` scans tags; `parser.header_hasher` (FNV) remains for response-side modules), `mime.zig` (comptime extension -> Content-Type switch).
