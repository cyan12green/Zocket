...
High-performance TCP server in Zig 0.16.0-dev (pinned in `build.zig.zon`). Through M14: multi-reactor epoll transport, HTTP/1.1, config-driven phase pipeline (`src/dsl/`, `src/runtime/`), static files, reverse proxy, observability, kernel-level optimizations. M15 (benchmark-driven hardening): on-demand request-buffer growth, nginx `open_file_cache`-style static fd + content cache, connection pooling with embedded buffers, request bump arena, cached Date header, experimental io_uring backend. M17 (TLS 1.3 server core, `src/tls/`): a native Zig TLS 1.3 server (ECDSA P-256/P-384 certs, X25519 ECDHE, AES-128-GCM/ChaCha20-Poly1305/AES-256-GCM, ALPN h2 + http/1.1, HRR, in-place record decryption) — no OpenSSL dependency; the std TLS client and openssl s_client both interop (verified end-to-end: handshake, encrypted round trip, close_notify). M18 (TLS in the reactor): per-connection detection (ClientHello vs h2 preface vs h1), config `tls` section, h1 + h2 over TLS, stateless session tickets + PSK resumption (openssl `Reused, TLSv1.3`), and HTTPS benchmarks vs nginx (Zocket leads the multiplexed workloads 1.1-2.9x, m=1 parity — `bench/tlsbench.sh`). In the nginx/actix/Bun/Caddy/httpx comparison (bench/compare-servers.sh) Zocket leads every measured workload. Future base for a hot-reloadable, nginx-style config-driven HTTP server.
...
- Daemon control (`--start`/`--stop`/`--status` with `--pidfile`; the daemon records `<pidfile>.state` — config path, optimize mode, port, threads, project root — for reloads). Reloads: `--reload-soft` sends SIGHUP for the in-process runtime reparse (only works when the daemon runs a runtime `--config`); `--reload-hard` rebuilds with `zig build -Doptimize=<state> -Dconfig=<state.config_path>` (comptime validation — bad configs abort with the old daemon untouched), execs the fresh `zig-out/bin/zocket --start`, then SIGTERMs the old daemon, which drains (graceful stop: stops accepting, closes its SO_REUSEPORT listeners, finishes connections within 30 s). The comptime embed (`@embedFile`) can only reach files inside the project tree (no dotdirs), so reload configs must live in the repo.
