...
//! side), paired with bench/reqresp_bench.zig (Zocket side). The variant
...
const body_1k = "The quick brown fox jumps over the lazy dog. " ** 22;
...
const response_variants = [_]ResponseVariant{
    .{ .name = "empty", .code = 200, .headers = &.{}, .body = "" },
    .{
        .name = "small",
        .code = 200,
...
var parse_parser: httpx.parser.Parser = undefined;

fn parseOpFor(comptime v: RequestVariant) void {
    _ = parse_parser.feed(v.wire) catch unreachable;
    parse_parser.reset();
}

var resp_out: std.ArrayList(u8) = .empty;

fn respondOpImpl(comptime v: ResponseVariant) void {
    var resp = httpx.response.Response.init(allocator, v.code);
    defer resp.deinit();
    var i: usize = 0;
    while (i < v.headers.len) : (i += 2) {
        resp.headers.set(v.headers[i], v.headers[i + 1]) catch unreachable;
    }
    resp.body = v.body;
    resp_out.clearRetainingCapacity();
    resp.serialize(ListWriter{ .allocator = allocator, .list = &resp_out }) catch unreachable;
}
...
pub fn main() !void {
    ...
    std.debug.print("=== httpx.zig request/response micro-benchmark (ReleaseFast) ===\n\n", .{});

    parse_parser = httpx.parser.Parser.init(allocator);
