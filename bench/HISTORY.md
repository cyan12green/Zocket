# Zocket benchmark history

Milestone-by-milestone benchmark commentary, moved out of `bench/BENCH.md`
when that document became the standing methodology + current-results
reference. The numbers below reflect the tree at the time each milestone was
reached; treat them as historical context. For how benchmarks are run today,
parameters and standing results, see `bench/BENCH.md`.
...
## Cross-server comparison: Zocket vs httpx.zig
...
- Both servers: ReleaseFast, GET / -> 200 with an empty body
  (Zocket default config; httpx bench server with `ctx.text("")`),
...
| metric | Zocket (this repo) | httpx.zig | ratio |
|---|---:|---:|---:|
...
as "Zocket's architecture sustains ~44x this build's server throughput
on this workload", not as a general statement about httpx.zig's ceiling.

### Request/response parsing micro-benchmark (Zocket vs httpx.zig)
...
| operation | Zocket ns/op (avg) | httpx.zig ns/op (avg) | ratio |
|---|---:|---:|---:|
...
- Zocket: parse state machine over a zero-copy connection buffer with
...
per-request CPU cost alone exceeds Zocket's entire keep-alive request
...
### Parameterized request/response matrix (Zocket vs httpx.zig)
...
| operation | variant | Zocket | httpx.zig | ratio |
|---|---:|---:|---:|---:|
...
- The parse gap grows with request complexity (44x minimal -> ~90x at 8
  ...
  Zocket's is a comptime-hashed append into reused storage.
- The response gap is the most extreme: the **empty** response builds in
  ...
  serialize path formats through an allocating printer, while Zocket's
  builder is a stack struct with comptime status/header strings.
- Both codebases scale linearly-ish with header count; Zocket's slope
...
## Cross-framework server comparison (Zocket vs actix-web / Bun.serve / httpx.zig)
...
Servers: Zocket (4 reactors, default config), actix-web (4 tokio
...
| workload | conns | Zocket | actix-web | Bun.serve | httpx.zig |
|---|---|---:|---:|---:|---:|
...
**Readings**:
- actix-web is the fastest at every point (1.65-1.78x Zocket). It is a
...
- Zocket is solidly second (Bun ~0.5-0.6x of us, httpx ~0.03x).
...
Zocket absolute numbers are lower than the earlier two-server
...
| cell | server | reqs/sec | vs Zocket | p50 | p95 | p99 | throughput |
|---|---|---:|---:|---:|---:|---:|---:|
| GET / empty, c=100 | Zocket | 292,645 | 1.00x | 0.23 ms | 0.95 ms | 2.29 ms | 18 MB/s |
...
| POST /echo 1 KB, c=10 | Zocket | 166,345 | 1.00x | 0.05 ms | 0.11 ms | 0.15 ms | 181 MB/s |
...
**Readings**:
- Small-payload echo (1 KB / 8 KB): Zocket is at parity or ahead of
...
| cell | server | reqs/sec | vs Zocket | p50 | p95 | p99 | throughput |
|---|---|---:|---:|---:|---:|---:|---:|
| GET / empty, c=100 | Zocket | 303,124 | 1.00x | 0.21 ms | 0.93 ms | 2.64 ms | 19 MB/s |
| | actix-web | 286,453 | 0.95x | 0.25 ms | 0.89 ms | 1.99 ms | 22 MB/s |
| | Bun.serve | 93,718 | 0.31x | 1.09 ms | 1.41 ms | 1.69 ms | 7 MB/s |
| POST /echo 1 KB, c=10 | Zocket | 168,916 | 1.00x | 0.05 ms | 0.10 ms | 0.15 ms | 184 MB/s |
| | actix-web | 130,845 | 0.77x | 0.07 ms | 0.14 ms | 0.20 ms | 144 MB/s |
| POST /echo 1 KB, c=100 | Zocket | 252,824 | 1.00x | 0.27 ms | 1.11 ms | 2.37 ms | 275 MB/s |
| | actix-web | 219,271 | 0.87x | 0.36 ms | 1.07 ms | 1.93 ms | 242 MB/s |
| POST /echo 1 KB, c=1000 | Zocket | 202,943 | 1.00x | 4.21 ms | 10.29 ms | 15.09 ms | 220 MB/s |
| | actix-web | 190,300 | 0.94x | 4.61 ms | 10.59 ms | 14.79 ms | 208 MB/s |
| POST /echo 8 KB, c=10 | Zocket | 113,043 | 1.00x | 0.07 ms | 0.16 ms | 0.24 ms | 933 MB/s |
| | actix-web | 93,300 | 0.83x | 0.09 ms | 0.20 ms | 0.32 ms | 772 MB/s |
| POST /echo 8 KB, c=100 | Zocket | 145,267 | 1.00x | 0.47 ms | 1.95 ms | 3.51 ms | 1199 MB/s |
| | actix-web | 127,126 | 0.88x | 0.61 ms | 1.92 ms | 3.18 ms | 1051 MB/s |
| POST /echo 8 KB, c=1000 | Zocket | 96,817 | 1.00x | 9.08 ms | 20.69 ms | 29.54 ms | 794 MB/s |
| | actix-web | 92,989 | 0.96x | 9.58 ms | 20.91 ms | 28.92 ms | 767 MB/s |
| POST /echo 64 KB, c=10 | Zocket | 54,405 | 1.00x | 0.16 ms | 0.32 ms | 0.47 ms | 3569 MB/s |
| | actix-web | 45,071 | 0.83x | 0.18 ms | 0.41 ms | 0.69 ms | 2957 MB/s |
| | Bun.serve | 24,063 | 0.44x | 0.33 ms | 0.82 ms | 1.03 ms | 1580 MB/s |
| POST /echo 64 KB, c=100 | Zocket | 29,189 | 1.00x | 2.64 ms | 8.56 ms | 13.11 ms | 1915 MB/s |
| | actix-web | 25,733 | 0.88x | 3.09 ms | 9.29 ms | 13.75 ms | 1688 MB/s |
| | Bun.serve | 21,922 | 0.75x | 4.28 ms | 6.03 ms | 6.65 ms | 1439 MB/s |
| POST /echo 64 KB, c=1000 | Zocket | 21,141 | 1.00x | 41.79 ms | 89.61 ms | 121.19 ms | 1374 MB/s |
| | actix-web | 19,289 | 0.91x | 46.59 ms | 99.65 ms | 131.26 ms | 1249 MB/s |
| | Bun.serve | 20,863 | 0.99x | 47.32 ms | 53.75 ms | 56.35 ms | 1363 MB/s |

**After the fix Zocket leads every cell** (actix 0.77-0.96x, i.e. us
...
| cell | server | reqs/sec | vs Zocket | p50 | p95 | p99 | throughput |
|---|---|---:|---:|---:|---:|---:|---:|
| GET / empty, c=100 | Zocket | 259,321 | 1.00x | 0.25 ms | 1.10 ms | 2.85 ms | 16 MB/s |
...
| POST /echo 1 KB, c=10 | Zocket | 159,837 | 1.00x | 0.05 ms | 0.11 ms | 0.16 ms | 174 MB/s |
...
| POST /echo 1 KB, c=100 | Zocket | 238,577 | 1.00x | 0.28 ms | 1.18 ms | 2.52 ms | 260 MB/s |
...
| POST /echo 1 KB, c=1000 | Zocket | 208,925 | 1.00x | 4.15 ms | 9.88 ms | 13.81 ms | 226 MB/s |
...
| POST /echo 8 KB, c=10 | Zocket | 107,454 | 1.00x | 0.08 ms | 0.17 ms | 0.25 ms | 887 MB/s |
...
| POST /echo 8 KB, c=100 | Zocket | 137,636 | 1.00x | 0.50 ms | 2.03 ms | 3.60 ms | 1137 MB/s |
...
| POST /echo 8 KB, c=1000 | Zocket | 99,044 | 1.00x | 8.93 ms | 20.19 ms | 27.97 ms | 811 MB/s |
...
| POST /echo 64 KB, c=10 | Zocket | 55,861 | 1.00x | 0.15 ms | 0.31 ms | 0.48 ms | 3665 MB/s |
...
| POST /echo 64 KB, c=100 | Zocket | 29,155 | 1.00x | 2.67 ms | 8.50 ms | 12.88 ms | 1911 MB/s |
...
| POST /echo 64 KB, c=1000 | Zocket | 22,178 | 1.00x | 40.83 ms | 79.37 ms | 102.08 ms | 1440 MB/s |
...
"static") at GET /static: Zocket via its static module (read loop below
...
| | Zocket | 47,870 | 1.00x | 2.02 ms | 2.92 ms | 3.85 ms | 59 MB/s |
...
| | Zocket | 47,116 | 1.00x | 20.56 ms | 25.00 ms | 30.18 ms | 57 MB/s |
| | Caddy | 35,783 | 0.76x | 29.64 ms | 43.19 ms | 50.56 ms | 44 MB/s |
| | httpx.zig | 5,649 | 0.12x | 1.20 ms | 370.56 ms | 2253.97 ms | 5 MB/s |
| GET /static 1 MB, c=100 | Zocket | 11,082 | 1.00x | 8.61 ms | 13.95 ms | 17.93 ms | 11614 MB/s |
...
| GET /static 1 MB, c=1000 | Zocket | 10,533 | 1.00x | 93.11 ms | 112.64 ms | 127.94 ms | 11001 MB/s |
...
- Small static (1 KB) is nginx's home turf: 4.6x/3.8x - its cached fast path
  ...
  behind).
- Large static (1 MB) flips the table: Zocket wins on sendfile +
...
4.6x Zocket (219.9k vs 47.9k co-resident; 233k vs 67k isolated).
nginx's per-request recipe for that path, versus ours at the time:

| step | nginx | Zocket (before) |
|---|---|---|
...
| | Zocket | 146,041 | 1.00x | 0.58 ms | 1.50 ms | 2.91 ms | 179 MB/s |
...
| | Zocket | 141,586 | 1.00x | 6.47 ms | 12.02 ms | 16.67 ms | 172 MB/s |
...
| GET /static 1 MB, c=100 | Zocket | 11,543 | 1.00x | 6.97 ms | 20.15 ms | 31.56 ms | 12083 MB/s |
...
| GET /static 1 MB, c=1000 | Zocket | 10,196 | 1.00x | 86.08 ms | 194.13 ms | 345.51 ms | 10560 MB/s |
...
Zocket now takes the 1 MB cell outright (sendfile + zero-copy writev,
...
| workload (c=100) | Zocket | nginx | vs |
|---|---:|---:|---:|
...
| workload | Zocket | nginx (sendfile on) | vs |
|---|---:|---:|---:|
...
3. **Cached Date header**: implemented (nginx ngx_cached_http_time
   ...
   plus a comptime "Server: Zocket" header. Responses now match
...
| workload (c=100, 12-rep interleaved medians) | Zocket | nginx | vs |
|---|---:|---:|---:|
...
with the same response shape put Zocket ahead. The nginx-feature
...
*Archive note: the original `bench/BENCH.md` opened with a machine header
(Date: 2026-08-10, 4-core i7-1185G7, Ubuntu 24.04, powersave governor) that
applies to all Milestone 1-15 numbers above. Later runs reuse the same box.*
